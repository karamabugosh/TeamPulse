...
You will receive standup data as JSON inside <standup_data> tags. Your job
has three parts:
...
3. Extract explicit BLOCKERS: anything a person flagged as blocking their
   work. Where severity/dependency were already reported by the person,
   preserve them exactly — do not override or reinterpret them.
...
- confidence must be a number between 0 and 1 (e.g. 0.85, not 85).
...
export function buildUserPrompt(
  ...
  const filteredResponses = responses.map((r) => ({
    userId: r.userId,
    answers: r.answers.filter((a) => a.answerType !== 'skip' && a.answerText),
  }));
