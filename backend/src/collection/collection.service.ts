...
export class CollectionService implements CollectionGateway {
  ...
  async startConversation(
    ...
    // Temporary behaviour:
    // Existing answers are deleted until StandupRun is added to the schema.
    if (session?.isCompleted) {
  ...
  async submitAnswer(
    ...
    if (!session || session.isCompleted) {
      this.logger.warn(
        `User ${userId} attempted to answer without an active conversation.`,
      );
      return;
    }
    ...
    if (session.currentQuestionId !== questionId) {
      this.logger.warn(
        `User ${userId} answered question ${questionId}, but their current question is ${session.currentQuestionId}.`,
      );
    }
  ...
  async finishConversation(userId: string): Promise<void> {
    ...
    if (!session) {
      this.logger.warn(
        `Cannot finish conversation because no session exists for user ${userId}.`,
      );
      return;
    }
  ...
  async getCompletedStandupResponses(): Promise<StandupResponse[]> {
    ...
    for (const session of completedSessions) {
      ...
      responses.push({
        ...
        // Replace this with the Slack display name once User is added
